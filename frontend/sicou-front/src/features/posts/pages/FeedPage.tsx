    ...
    <div className="mx-auto max-w-4xl">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        ...
        <div className="flex gap-2">
          ...
          <button type="button" onClick={openComposer} className="inline-flex items-center justify-center gap-2 rounded-lg bg-slate-900 px-4 py-2 text-sm font-semibold text-white hover:bg-slate-700">
      ...
      {errorMessage && !isComposerOpen && (
        <div className="mt-5 rounded-xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-700">{errorMessage}</div>
      )}

      <div className="mt-6 space-y-4">
        {isLoading && posts.length === 0 && <div className="py-10 text-center text-sm text-slate-500">Carregando publicações...</div>}
        {!isLoading && posts.length === 0 && <div className="rounded-2xl border border-dashed border-slate-300 bg-white px-6 py-12 text-center"><p className="font-semibold text-slate-700">Ainda não há publicações.</p><p className="mt-1 text-sm text-slate-500">Seja a primeira pessoa a compartilhar uma novidade.</p></div>}
        {posts.map((post) => (
              ...
              <div className="flex items-start justify-between gap-4">
                <div>
                  <div className="flex flex-wrap items-center gap-2">
                    {post.isPinned && <span className="inline-flex items-center gap-1 rounded-full bg-amber-50 px-2.5 py-1 text-xs font-semibold text-amber-700"><Pin className="h-3.5 w-3.5" />Fixado</span>}
                    <p className="text-xs font-medium text-slate-500">{post.companyName}</p>
                  </div>
                  <h2 className="mt-2 text-lg font-bold text-slate-900">{post.title}</h2>
                </div>
                <div className="flex gap-1">
                  {canModeratePost(post) && <button type="button" aria-label="Alternar destaque" onClick={() => void togglePin(post)} className="rounded-lg p-2 text-slate-500 hover:bg-slate-100"><Pin className={`h-4 w-4 ${post.isPinned ? 'fill-amber-500 text-amber-600' : ''}`} /></button>}
                  {canEdit(post) && <button type="button" aria-label="Editar" onClick={() => beginEdit(post)} className="rounded-lg p-2 text-slate-500 hover:bg-slate-100"><Pencil className="h-4 w-4" /></button>}
                  {canEdit(post) && <button type="button" aria-label="Excluir" onClick={() => void remove(post)} className="rounded-lg p-2 text-rose-600 hover:bg-rose-50"><Trash2 className="h-4 w-4" /></button>}
                </div>
              </div>
              <p className="mt-3 whitespace-pre-wrap text-sm leading-6 text-slate-700">{post.content}</p>
              <p className="mt-4 text-xs text-slate-500">Por {post.authorName} · {formatDate(post.createdAt)}{post.updatedAt ? ' · editado' : ''}</p>
            </div>
          </article>
        ))}
      </div>
      ...
      {isComposerOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true" aria-labelledby="post-composer-title">
          <button type="button" onClick={closeComposer} className="absolute inset-0 bg-slate-950/40" aria-label="Fechar janela de publicação" />
          <div className="relative z-10 max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white shadow-2xl ring-1 ring-slate-200">
            <div className="sticky top-0 flex items-start justify-between gap-4 border-b border-slate-200 bg-white px-6 py-5">
              <div>
                <h2 id="post-composer-title" className="text-lg font-semibold text-slate-900">{editingPost ? 'Editar publicação' : 'Nova publicação'}</h2>
                <p className="mt-1 text-sm text-slate-500">Compartilhe um comunicado com sua empresa.</p>
              </div>
            ...
            <form onSubmit={(event) => void submit(event)} className="p-6">
              {errorMessage && <div className="mb-5 rounded-xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-700">{errorMessage}</div>}
              <input required maxLength={200} value={form.title} onChange={(event) => setForm((value) => ({ ...value, title: event.target.value }))} placeholder="Título do comunicado" className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm" />
              <textarea required maxLength={5000} value={form.content} onChange={(event) => setForm((value) => ({ ...value, content: event.target.value }))} placeholder="Escreva a sua mensagem..." rows={7} className="mt-3 w-full resize-y rounded-lg border border-slate-300 px-3 py-2 text-sm" />
              {isSuperAdmin && !editingPost && <label className="mt-3 inline-flex items-center gap-2 text-sm font-medium text-slate-700"><input type="checkbox" checked={form.publishToAllCompanies} onChange={(event) => setForm((value) => ({ ...value, publishToAllCompanies: event.target.checked }))} className="h-4 w-4 rounded border-slate-300" />Publicar em todas as empresas</label>}
              {imagePreview && <img src={imagePreview} alt="Prévia da imagem" className="mt-4 max-h-64 w-full rounded-xl bg-slate-100 object-contain" />}
              {!imagePreview && editingPost?.imageUrl && !form.removeImage && <img src={postImageUrl(editingPost.imageUrl) ?? undefined} alt="Imagem atual" className="mt-4 max-h-64 w-full rounded-xl bg-slate-100 object-contain" />}
